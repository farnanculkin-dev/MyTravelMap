...
import { loadTripsSharedWithMe, type SharedTripSummary } from '../lib/SharingRuntime'
import ConnectedProfilePanel from './ConnectedProfilePanel'

export default function ConnectionsPanel({ atlasId }: { atlasId: string }) {
  const [connections, setConnections] = useState<AtlasConnection[]>([])
  const [sharedWithMe, setSharedWithMe] = useState<SharedTripSummary[]>([])
  ...
  async function refresh() {
    try {
      const [nextConnections, nextShared] = await Promise.all([loadAtlasConnections(atlasId), loadTripsSharedWithMe()])
      setConnections(nextConnections); setSharedWithMe(nextShared)
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await refresh()
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Connection could not be added.') }
  ...
  async function handleInvite(connection: AtlasConnection) {
    ...
    try { await inviteAtlasConnection(connection.id, address); setMessage(`Invitation sent to ${connection.displayName}.`); await refresh() }
  ...
  return <section className="settings-card connections-card" aria-labelledby="connections-title">
    ...
    <form className="connection-form" onSubmit={handleSubmit}>
      <label>Name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Gráinne" required /></label>
      <label>Relationship <span>(optional)</span><input value={relationship} onChange={(event) => setRelationship(event.target.value)} placeholder="e.g. Mum, sister, family friend" /></label>
    ...
    <div className="connections-list">
      {connections.length === 0 ? <p className="trip-form-help">No wider connections added yet.</p> : connections.map((connection) => <article key={connection.id} className="connection-row">
        <button className="connection-person-button" type="button" disabled={connection.status !== 'connected'} onClick={() => setSelectedConnectionId(connection.id)}>
          <strong>{connection.displayName}</strong><span>{connection.relationshipLabel || (connection.connectionType === 'friend' ? 'Friend' : connection.connectionType === 'family' ? 'Connected family' : 'Relative')}</span>
        </button>
        ...
        {connection.status === 'connected' ? <button className="secondary-btn" type="button" onClick={() => setSelectedConnectionId(connection.id)}>View profile</button> : <button className="secondary-btn" type="button" onClick={() => void handleInvite(connection)}>{connection.status === 'invited' ? 'Resend invite' : 'Invite'}</button>}
      </article>)}
    </div>

    {sharedWithMe.length > 0 && <div className="shared-with-me-section"><h3>Shared with me</h3><p className="trip-form-help">Trips other connected people have explicitly shared with your account.</p><div className="shared-trip-grid">{sharedWithMe.map((trip) => <article className="shared-trip-card" key={`${trip.connectionId}-${trip.tripId}`}><p className="auth-eyebrow">From {trip.sharedByName || 'a connection'}</p><h4>{trip.title}</h4><p>{trip.startDate || 'Date not added'}</p>{trip.description && <p>{trip.description}</p>}</article>)}</div></div>}
  </section>
}
