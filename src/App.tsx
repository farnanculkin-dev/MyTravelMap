...
import { consumePendingConnectionInvitation } from './lib/ConnectionRuntime'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    consumePendingInvitation()
      .then(() => consumePendingConnectionInvitation())
  ...
  return <div className="app">
    ...
    {section === 'settings' ? <main className="settings-screen"><div className="settings-title"><p className="auth-eyebrow">Family Atlas</p><h1>Settings</h1><p>Manage household access and your wider trusted circle without cluttering your travel pages.</p></div>{membership.role === 'admin' ? <MemberInvitePanel atlasId={membership.atlasId} profiles={activeProfiles} /> : <section className="settings-card"><h2>Household access</h2><p>Only an Atlas administrator can invite another household member.</p></section>}<ConnectionsPanel atlasId={membership.atlasId} /></main> : section === 'trips' ? <TripsScreen atlasId={membership.atlasId} profiles={activeProfiles} onBack={handleHome} initialParticipantId={newTripPersonId||undefined} /> : !profile ? <HomeProfiles profiles={activeProfiles} onSelect={handleSelectProfile} groupImage={cloudData?.groupImage} profileImages={cloudData?.profileImages} onUploadImage={cloudData ? uploadImage : undefined} /> : <>
