...
create unique index if not exists atlas_invitations_one_pending_profile
  on public.atlas_invitations (atlas_id, profile_id)
  where status = 'pending';

alter table public.atlas_invitations enable row level security;

create or replace function public.create_atlas_member_invitation(
  p_atlas_id uuid,
  p_profile_key text,
  p_email text
)
returns public.atlas_invitations
language plpgsql
security definer
...
declare
  ...
  invitation public.atlas_invitations%rowtype;
  normalized_email text := lower(trim(p_email));
begin
  if caller_id is null then raise exception 'Authentication required'; end if;
  if not exists (
    select 1 from public.atlas_members
    where atlas_id = p_atlas_id and user_id = caller_id and role = 'admin'
  ) then raise exception 'Atlas administrator access required'; end if;
  ...
  where atlas_id = p_atlas_id and profile_key = p_profile_key;
  ...
  insert into public.atlas_invitations (atlas_id, profile_id, email, role, invited_by)
  values (p_atlas_id, target_profile.id, normalized_email, 'member', caller_id)
  on conflict (atlas_id, profile_id) where status = 'pending'
  do update set email = excluded.email, invited_by = excluded.invited_by,
...
begin
  ...
  select * into pending from public.atlas_invitations
  where lower(email) = caller_email and status = 'pending' and expires_at > now()
  order by created_at desc limit 1 for update;
  if not found then return; end if;

  update public.profiles set linked_user_id = caller_id
  where id = pending.profile_id and atlas_id = pending.atlas_id and linked_user_id is null;
  ...
  on conflict (atlas_id, user_id) do update set role = excluded.role;
